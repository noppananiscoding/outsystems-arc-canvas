...
export default function CanvasToolbar({
  ...
  const { projectName, violations, setProjectName, importArchitecture, validateAll, aiMode, mcpConnected } =
  ...
  return (
    <>
      <div className="fixed top-0 left-0 right-0 flex items-center gap-2 px-4 py-2 bg-gray-900 border-b border-gray-700 z-50 flex-wrap">
        ...
        {/* Connect AI Agent button */}
        <button
          onClick={() => setShowMCPModal(true)}
          title="Connect AI Agent via MCP"
          className={`flex items-center gap-1.5 text-sm px-3 py-1.5 rounded transition-colors cursor-pointer flex-shrink-0 relative ${
            mcpConnected
      ...
      {showMCPModal && <MCPConnectModal onClose={() => setShowMCPModal(false)} />}
    </>
  );
}
